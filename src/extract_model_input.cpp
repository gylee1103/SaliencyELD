 ...
 * -------------------------------------------------------------*/
void ExtractInitialFeatureDistanceMap(pair<const int, Region>& creg_pair,
        const Region &query_region, GTOR& grid2region,
        float input_map[DIM_MODEL_FEED][GRID_SIZE][GRID_SIZE]) {
	vector<pair<int, int> > dominate_gcells;
...
void GenerateInitialFeatureDistanceMapDatum(REGION_INFOS& region_infos,
        GTOR& grid2region, const Region& query_region, Datum& output) {
	float tmp_float_map[DIM_MODEL_FEED][GRID_SIZE][GRID_SIZE] = {0};
	for (auto& creg_pair: region_infos) {
		ExtractInitialFeatureDistanceMap(creg_pair, query_region,
 ...
 * ---------------------------------------------------------*/
void GenerateSlicRegions(Mat& image, REGION_INFOS& region_infos,
                         GTOR& grid2region) {
    ...
    float* input = (float*)malloc(sizeof(float)*height*width*DIM_COLOR);
 ...
 * ----------------------------------------------*/
void GenerateImageDatum(Mat& _cv_img, vector<Datum>& img_vec, 
                        const int CROP_SIZE) {
    Datum datum;
    Mat resized_img;
    resize(_cv_img, resized_img, cv::Size(CROP_SIZE, CROP_SIZE));
    img_vec.push_back(datum);
    ...
    string buffer(3*CROP_SIZE*CROP_SIZE, 0);
    img_vec[0].set_data(buffer);
    string* data_ptr = img_vec[0].mutable_data();
